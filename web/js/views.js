...
export async function HomeView() {
  const box = el("section", { class:"card" },
    ...
    el("p", {}, "Créez et partagez des fiches pédagogiques. L’app fonctionne hors-ligne et exporte vos données. "),
    el("p", { class:"meta" }, "Back/API (FastAPI/Render), Auth Supabase, Sheets/Drive, EmailJS en cours.")
  );
  return box;
}
...
export async function FichesView() {
  ...
  async function renderList() {
    ...
    const list = key ? (await listFiches({ niveau:key })) : fiches;
    grid.innerHTML = "";
    list.forEach(f => {
