...
(function(){
  // Jeu de données fictif pour tester l’interface
  const sampleFiches = [
    {
      id: '1',
  ...
  function filterFiches({ q = '', status = '' }) {
    return sampleFiches.filter(f => {
      const matchQ = !q || f.title.toLowerCase().includes(q.toLowerCase()) || f.resume.toLowerCase().includes(q.toLowerCase());
  ...
  const Fiches = {
    list: async ({ q, status, limit = 20, offset = 0 } = {}) => {
      ...
      // Implémente un découpage simple
    ...
    export: async (id, format) => {
      // Dans ce mode démo, on n’exporte rien mais on renvoie un succès
